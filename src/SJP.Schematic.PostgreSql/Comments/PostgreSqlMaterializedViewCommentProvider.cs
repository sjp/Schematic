...
public class PostgreSqlMaterializedViewCommentProvider : IDatabaseViewCommentProvider
{
    ...
    /// <summary>
    /// Enumerates all materialized view comments defined within a database.
    /// </summary>
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>An <see cref="OptionAsync{IDatabaseViewComments}" /> instance which holds the value of the materialized view's comments, if available.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="viewName"/> is <see langword="null" />.</exception>
    ...
    private static Option<string> GetFirstCommentByType(IReadOnlyDictionary<string, List<GetMaterializedViewComments.Result>> commentsByType, string objectType)
    {
    ...
    private static IReadOnlyDictionary<Identifier, Option<string>> GetCommentLookupByType(IReadOnlyDictionary<string, List<GetMaterializedViewComments.Result>> commentsByType, string objectType)
    {
        ArgumentNullException.ThrowIfNull(commentsByType);
        ArgumentException.ThrowIfNullOrWhiteSpace(objectType);
