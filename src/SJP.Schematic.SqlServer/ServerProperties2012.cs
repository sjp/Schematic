...
{
    internal sealed class ServerProperties2012 : IServerProperties2012
    {
        public ServerProperties2012(QueryResult.ServerProperties2012QueryResult query)
        {
            if (query == null)
            ...
            IsHadrEnabled = query.IsHadrEnabled.Match(static isHadrEnabled => Convert.ToBoolean(isHadrEnabled), static () => (bool?)null);
            IsIntegratedSecurityOnly = query.IsIntegratedSecurityOnly.Match(isIntegratedSecurityOnly => Convert.ToBoolean(isIntegratedSecurityOnly), static () => (bool?)null);
            IsLocalDB = query.IsLocalDB.Match(static isLocalDb => Convert.ToBoolean(isLocalDb), static () => (bool?)null);
