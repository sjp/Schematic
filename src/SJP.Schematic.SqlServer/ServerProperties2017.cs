...
{
    public ServerProperties2017(QueryResult.ServerProperties2017QueryResult query)
    {
        if (query == null)
        ...
        IsXTPSupported = query.IsXTPSupported.Match(static isXtpSupported => Convert.ToBoolean(isXtpSupported), static () => (bool?)null);
        LCID = query.LCID;
