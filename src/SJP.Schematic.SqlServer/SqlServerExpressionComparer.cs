...
namespace SJP.Schematic.SqlServer
{
    public sealed class SqlServerExpressionComparer : IEqualityComparer<string>
    {
        ...
        private bool TokensEqual(Token<ExpressionToken> x, Token<ExpressionToken> y)
        {
            if (x.Kind != y.Kind)
                return false;

            var comparer = x.Kind == ExpressionToken.String
                ? SqlStringComparer
                : Comparer;
        ...
        private static IReadOnlyList<Token<ExpressionToken>> StripWrappingParens(IReadOnlyList<Token<ExpressionToken>> tokens)
        {
            if (tokens == null)
                throw new ArgumentNullException(nameof(tokens));

            // copy to mutable result set
            var result = new List<Token<ExpressionToken>>();
            if (tokens.Count == 0)
            ...
            if (result[0].Kind == ExpressionToken.LParen && result[lastIndex].Kind == ExpressionToken.RParen)
            {
            ...
            {
                var token = result[i];
                if (token.Kind != ExpressionToken.Number)
                    continue;
                ...
                if (prevToken.Kind == ExpressionToken.LParen
                    && nextToken.Kind == ExpressionToken.RParen)
                {
                    // remove next first
                    result.RemoveAt(i + 1);
                    result.RemoveAt(i - 1);
