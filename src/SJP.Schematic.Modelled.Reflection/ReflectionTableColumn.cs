...
namespace SJP.Schematic.Modelled.Reflection
{
    public class ReflectionTableColumn : IDatabaseColumn
    {
        public ReflectionTableColumn(IDatabaseDialect dialect, PropertyInfo prop, Type declaredColumnType, bool isNullable)
        {
            ...
            if (clrType == null)
                throw new ArgumentNullException(nameof(declaredColumnType), $"The declared column type does not implement IDbType<T>. Check { prop.ReflectedType!.FullName }.{ prop.Name } and ensure that the column type { declaredColumnType.FullName } implements this interface.");
            ...
            if (autoIncrAttr != null)
            {
                if (!ValidAutoIncrementTypes.Contains(columnType.DataType))
                    throw new ArgumentNullException(nameof(declaredColumnType), $"The column { prop.ReflectedType!.FullName }.{ prop.Name } is declared as being auto incrementing, which is not supported on a '{ columnType.DataType }' data type.");
