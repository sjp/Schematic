    ...
    private async Task GenerateAsyncCore(string projectPath, string baseNamespace, CancellationToken cancellationToken)
    {
        ...
        foreach (var table in tables)
        {
            var tableComment = tableCommentsLookup.TryGetValue(table.Name, out var comments)
                ? Option<IRelationalDatabaseTableComments>.Some(comments)
        ...
        {
            var viewComment = viewCommentsLookup.TryGetValue(view.Name, out var comments)
                ? Option<IDatabaseViewComments>.Some(comments)
