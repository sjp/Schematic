...
internal sealed class OracleDatabaseViewProviderTests : OracleTest
{
    ...
    {
        await DbConnection.ExecuteAsync("create view db_test_view_1 as select 1 as dummy from dual", TestContext.CurrentContext.CancellationToken);

        await DbConnection.ExecuteAsync("create view view_test_view_1 as select 1 as test from dual", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("create table view_test_table_1 (table_id number)", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("create materialized view view_test_view_2 as select table_id as test from view_test_table_1", TestContext.CurrentContext.CancellationToken);
    }
    ...
    public async Task CleanUp()
    {
        ...
        await DbConnection.ExecuteAsync("drop materialized view view_test_view_2", TestContext.CurrentContext.CancellationToken);
