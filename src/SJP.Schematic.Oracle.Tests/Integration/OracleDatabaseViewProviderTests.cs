...
namespace SJP.Schematic.Oracle.Tests.Integration
{
    internal sealed class OracleDatabaseViewProviderTests : OracleTest
    {
        ...
        {
            await Connection.ExecuteAsync("create view db_test_view_1 as select 1 as dummy from dual", CancellationToken.None).ConfigureAwait(false);

            await Connection.ExecuteAsync("create view view_test_view_1 as select 1 as test from dual", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table view_test_table_1 (table_id number)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create materialized view view_test_view_2 as select table_id as test from view_test_table_1", CancellationToken.None).ConfigureAwait(false);
        }
        ...
        public async Task CleanUp()
        {
            ...
            await Connection.ExecuteAsync("drop materialized view view_test_view_2", CancellationToken.None).ConfigureAwait(false);
