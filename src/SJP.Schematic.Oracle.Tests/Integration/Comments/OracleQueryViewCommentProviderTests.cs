...
namespace SJP.Schematic.Oracle.Tests.Integration.Comments
{
    internal sealed class OracleQueryViewCommentProviderTests : OracleTest
    {
        ...
        {
            await Connection.ExecuteAsync("create table view_comment_table_1 ( test_column_1 number, test_column_2 number, test_column_3 number )", CancellationToken.None).ConfigureAwait(false);
            ...
            await Connection.ExecuteAsync("comment on table view_comment_view_2 is 'This is a test view comment.'", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("comment on column view_comment_view_2.test_column_2 is 'This is a column comment.'", CancellationToken.None).ConfigureAwait(false);
        }
