...
internal sealed class OracleTableCommentProviderTests : OracleTest
{
    ...
    {
        await DbConnection.ExecuteAsync("create table table_comment_table_1 ( test_column_1 number )", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("create table table_comment_table_2 ( test_column_1 number, test_column_2 number, test_column_3 number )", TestContext.CurrentContext.CancellationToken);

        await DbConnection.ExecuteAsync("comment on table table_comment_table_2 is 'This is a test table comment.'", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("comment on column table_comment_table_2.test_column_2 is 'This is a column comment.'", TestContext.CurrentContext.CancellationToken);
    }
