...
namespace SJP.Schematic.Oracle.Tests.Integration
{
    internal partial class OracleRelationalDatabaseTableProviderTests : OracleTest
    {
        ...
        public async Task Init()
        {
            ...
            await Connection.ExecuteAsync("create table db_test_table_1 ( title varchar2(200) )", CancellationToken.None).ConfigureAwait(false);

            await Connection.ExecuteAsync("create table table_test_table_1 ( test_column number )", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table table_test_table_2 ( test_column number not null primary key )", CancellationToken.None).ConfigureAwait(false);
...
create table table_test_table_4 (
    ...
    constraint pk_test_table_4 primary key (first_name, last_name, middle_name)
            ...
            await Connection.ExecuteAsync("create table table_test_table_5 ( test_column number not null unique )", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
...
create table table_test_table_7 (
    ...
    constraint uk_test_table_7 unique (first_name, last_name, middle_name)
            ...
            await Connection.ExecuteAsync("create table table_test_table_8 (test_column number)", CancellationToken.None).ConfigureAwait(false);
...
create table table_test_table_30 (
    ...
    constraint fk_test_table_30 foreign key (first_name_child) references table_test_table_15 (first_name_parent)
            ...
            await Connection.ExecuteAsync("alter table table_test_table_30 disable constraint fk_test_table_30", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
create table table_test_table_31 (
    ...
    constraint fk_test_table_31 foreign key (last_name_child, middle_name_child) references table_test_table_15 (last_name_parent, middle_name_parent)
            ...
            await Connection.ExecuteAsync("alter table table_test_table_31 disable constraint fk_test_table_31", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
create table table_test_table_32 (
    test_column number not null,
    constraint ck_test_table_32 check (test_column > 1)
)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("alter table table_test_table_32 disable constraint ck_test_table_32", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table table_test_table_33 ( test_column number default 1 not null )", CancellationToken.None).ConfigureAwait(false);
    ...
    test_column_3 as (test_column_1 + test_column_2)
            ...
            await Connection.ExecuteAsync("create table table_test_table_35 ( test_column number primary key )", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table trigger_test_table_1 (table_id number primary key not null)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table trigger_test_table_2 (table_id number primary key not null)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
