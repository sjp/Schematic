...
internal sealed class ServerProperties2017Tests : SqlServer2017Test
{
    [Test]
    public void Ctor_GivenNullDto_ThrowsArgumentNullException()
    {
        Assert.That(() => new ServerProperties2017(null), Throws.ArgumentNullException);
    }

    [Test]
    public async Task BuildClrVersion_PropertyGet_IsNotNull()
    {
        var serverProps = await Dialect.GetServerProperties2017(Config.ConnectionFactory).ConfigureAwait(false);
