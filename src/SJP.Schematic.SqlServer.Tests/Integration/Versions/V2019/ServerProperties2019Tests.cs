...
internal sealed class ServerProperties2019Tests : SqlServer2019Test
{
    [Test]
    public void Ctor_GivenNullDto_ThrowsArgumentNullException()
    {
        Assert.That(() => new ServerProperties2019(null), Throws.ArgumentNullException);
    }

    [Test]
    public async Task BuildClrVersion_PropertyGet_IsNotNull()
    {
        var serverProps = await Dialect.GetServerProperties2019(Config.ConnectionFactory).ConfigureAwait(false);
    ...
    [Test]
    public async Task IsBigDataCluster_PropertyGet_IsNotNull()
    {
        var serverProps = await Dialect.GetServerProperties2019(Config.ConnectionFactory).ConfigureAwait(false);

        Assert.That(serverProps.IsBigDataCluster, Is.Not.Null);
    }
    ...
    [Test]
    public async Task IsPolyBaseInstalled_PropertyGet_IsNotNull()
    {
        var serverProps = await Dialect.GetServerProperties2019(Config.ConnectionFactory).ConfigureAwait(false);

        Assert.That(serverProps.IsPolyBaseInstalled, Is.Not.Null);
    }
