...
{
    internal sealed class ServerProperties2012Tests : SqlServer2014Test
    {
        [Test]
        public void Ctor_GivenNullDto_ThrowsArgumentNullException()
        {
            Assert.That(() => new ServerProperties2012(null), Throws.ArgumentNullException);
        }

        [Test]
        public async Task BuildClrVersion_PropertyGet_IsNotNull()
        {
            var serverProps = await Dialect.GetServerProperties2012(Config.ConnectionFactory).ConfigureAwait(false);
        ...
        [Test]
        public async Task ProductBuildType_PropertyGet_IsNotNull()
        {
            var serverProps = await Dialect.GetServerProperties2012(Config.ConnectionFactory).ConfigureAwait(false);

            Assert.That(serverProps.ProductBuildType, Is.Not.Null);
        }
