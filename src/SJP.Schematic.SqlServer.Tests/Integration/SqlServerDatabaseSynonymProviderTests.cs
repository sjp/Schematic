...
internal sealed class SqlServerDatabaseSynonymProviderTests : SqlServerTest
{
    ...
    {
        await DbConnection.ExecuteAsync("create synonym db_test_synonym_1 for sys.tables", TestContext.CurrentContext.CancellationToken);

        await DbConnection.ExecuteAsync("create view synonym_test_view_1 as select 1 as test", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("create table synonym_test_table_1 (table_id int primary key not null)", TestContext.CurrentContext.CancellationToken);
    ...
    public async Task GetSynonym_WhenSynonymPresentGivenLocalNameWithDifferentCase_ReturnsMatchingName()
    {
        ...
        var synonym = await SynonymProvider.GetSynonym(inputName).UnwrapSomeAsync();
    ...
    public async Task GetSynonym_WhenSynonymPresent_IssuesOneQuery()
    {
        ...
        var synonymProvider = new SqlServerDatabaseSynonymProvider(countingConnectionFactory, IdentifierDefaults);
