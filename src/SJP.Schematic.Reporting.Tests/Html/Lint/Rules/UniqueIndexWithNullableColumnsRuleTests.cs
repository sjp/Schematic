...
internal static class UniqueIndexWithNullableColumnsRuleTests
{
    ...
    [Test]
    public static async Task AnalyseTables_GivenTableWithNullableColumnsInUniqueIndex_ProducesMessageWithVisibleTableName()
    {
        var rule = new UniqueIndexWithNullableColumnsRule(RuleLevel.Error);
        var tableName = Identifier.CreateQualifiedIdentifier("test_schema", "test_table");

        var testColumn = new DatabaseColumn("test_column_1", Mock.Of<IDbType>(), true, null, null);
    ...
    public static async Task AnalyseTables_GivenTableWithNoNullableColumnsInUniqueIndex_ProducesNoMessages()
    {
        ...
        var testColumn = new DatabaseColumn("test_column_1", Mock.Of<IDbType>(), false, null, null);
