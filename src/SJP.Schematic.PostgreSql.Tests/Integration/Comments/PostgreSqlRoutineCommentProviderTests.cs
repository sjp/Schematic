...
LANGUAGE PLPGSQL").ConfigureAwait(false);
        ...
        {
            await Connection.ExecuteAsync("drop function db_comment_test_routine_1(integer)").ConfigureAwait(false);
        }
        ...
        {
            var routineComments = await RoutineCommentProvider.GetAllRoutineComments().ConfigureAwait(false);
            var containsTestRoutine = routineComments.Any(t => t.RoutineName.LocalName == "db_comment_test_routine_1");
