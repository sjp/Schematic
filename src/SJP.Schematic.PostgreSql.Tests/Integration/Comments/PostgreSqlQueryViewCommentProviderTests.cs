...
internal sealed class PostgreSqlQueryViewCommentProviderTests : PostgreSqlTest
{
    ...
    {
        await DbConnection.ExecuteAsync("create table view_comment_table_1 (test_column_1 int primary key not null, test_column_2 int, test_column_3 int)", TestContext.CurrentContext.CancellationToken);

        await DbConnection.ExecuteAsync("create view view_comment_view_1 as select test_column_1, test_column_2, test_column_3 from view_comment_table_1", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("create view view_comment_view_2 as select test_column_1, test_column_2, test_column_3 from view_comment_table_1", TestContext.CurrentContext.CancellationToken);

        await DbConnection.ExecuteAsync("comment on view view_comment_view_2 is 'This is a test view.'", TestContext.CurrentContext.CancellationToken);
        await DbConnection.ExecuteAsync("comment on column view_comment_view_2.test_column_2 is 'This is a test view column.'", TestContext.CurrentContext.CancellationToken);
    }
