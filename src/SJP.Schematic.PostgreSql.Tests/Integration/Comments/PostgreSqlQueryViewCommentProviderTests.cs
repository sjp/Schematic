...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Comments
{
    ...
    {
        private IDatabaseViewCommentProvider ViewCommentProvider => new PostgreSqlQueryViewCommentProvider(Connection, IdentifierDefaults);

        [OneTimeSetUp]
        public async Task Init()
        {
            await Connection.ExecuteAsync("create table view_comment_table_1 (test_column_1 int primary key not null, test_column_2 int, test_column_3 int)").ConfigureAwait(false);

            await Connection.ExecuteAsync("create view view_comment_view_1 as select test_column_1, test_column_2, test_column_3 from view_comment_table_1").ConfigureAwait(false);
            await Connection.ExecuteAsync("create view view_comment_view_2 as select test_column_1, test_column_2, test_column_3 from view_comment_table_1").ConfigureAwait(false);

            await Connection.ExecuteAsync("comment on view view_comment_view_2 is 'This is a test view.'").ConfigureAwait(false);
            await Connection.ExecuteAsync("comment on column view_comment_view_2.test_column_2 is 'This is a test view column.'").ConfigureAwait(false);
        }
        ...
        public async Task CleanUp()
        {
            ...
            await Connection.ExecuteAsync("drop table view_comment_table_1").ConfigureAwait(false);
        }
        ...
        {
            var viewComments = await ViewCommentProvider.GetAllViewComments().ConfigureAwait(false);
            var containsTestView = viewComments.Any(t => t.ViewName.LocalName == "view_comment_view_1");
