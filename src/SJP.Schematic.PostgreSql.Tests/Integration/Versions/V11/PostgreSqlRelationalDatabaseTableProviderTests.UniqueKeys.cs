...
internal sealed partial class PostgreSqlRelationalDatabaseTableProviderTests : PostgreSql11Test
{
    ...
    {
        var table = await GetTableAsync("v11_table_test_table_5").ConfigureAwait(false);
    ...
    {
        var table = await GetTableAsync("v11_table_test_table_6").ConfigureAwait(false);
        var uk = table.UniqueKeys.Single();
    ...
    public async Task UniqueKeys_WhenGivenTableWithMultiColumnConstraintAsUniqueKey_ReturnsUniqueKeyWithColumnsInCorrectOrder()
    {
        ...
        var table = await GetTableAsync("v11_table_test_table_7").ConfigureAwait(false);
        var uk = table.UniqueKeys.Single();
