...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V10
{
    internal partial class PostgreSqlRelationalDatabaseTableProviderTests : PostgreSql10Test
    {
        ...
        {
            var table = await GetTableAsync("v10_table_test_table_8").ConfigureAwait(false);
        ...
        public async Task Indexes_WhenGivenTableWithMultiColumnIndex_ReturnsIndexWithColumnsInCorrectOrder()
        {
            ...
            var table = await GetTableAsync("v10_table_test_table_9").ConfigureAwait(false);
            var index = table.Indexes.Single();
            var indexColumns = index.Columns
                .Select(c => c.DependentColumns.Single())
        ...
        {
            var table = await GetTableAsync("v10_table_test_table_11").ConfigureAwait(false);
            var index = table.Indexes.Single();
        ...
        {
            var table = await GetTableAsync("v10_table_test_table_13").ConfigureAwait(false);
            var index = table.Indexes.Single();
