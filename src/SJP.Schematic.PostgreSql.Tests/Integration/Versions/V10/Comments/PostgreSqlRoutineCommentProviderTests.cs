...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V10.Comments
{
    internal sealed class PostgreSqlRoutineCommentProviderTests : PostgreSql10Test
    {
        ...
        {
            await Connection.ExecuteAsync(@"CREATE FUNCTION v10_comment_test_routine_1(val integer)
RETURNS integer AS $$
BEGIN
    RETURN val + 1;
END; $$
LANGUAGE PLPGSQL", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"CREATE FUNCTION v10_comment_test_routine_2(val integer)
RETURNS integer AS $$
BEGIN
    RETURN val + 1;
END; $$
LANGUAGE PLPGSQL", CancellationToken.None).ConfigureAwait(false);
            ...
            await Connection.ExecuteAsync("comment on function v10_comment_test_routine_2 (integer) is 'This is a test function.'", CancellationToken.None).ConfigureAwait(false);
        }

        [OneTimeTearDown]
        public async Task CleanUp()
        {
            await Connection.ExecuteAsync("drop function v10_comment_test_routine_1(integer)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop function v10_comment_test_routine_2(integer)", CancellationToken.None).ConfigureAwait(false);
        }
