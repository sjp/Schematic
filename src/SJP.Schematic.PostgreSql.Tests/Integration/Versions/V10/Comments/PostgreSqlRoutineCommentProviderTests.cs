...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V10.Comments
{
    internal sealed class PostgreSqlRoutineCommentProviderTests : PostgreSql10Test
    {
        ...
        {
            await DbConnection.ExecuteAsync(@"CREATE FUNCTION v10_comment_test_routine_1(val integer)
RETURNS integer AS $$
BEGIN
    RETURN val + 1;
END; $$
LANGUAGE PLPGSQL", CancellationToken.None).ConfigureAwait(false);
            await DbConnection.ExecuteAsync(@"CREATE FUNCTION v10_comment_test_routine_2(val integer)
RETURNS integer AS $$
BEGIN
    RETURN val + 1;
END; $$
LANGUAGE PLPGSQL", CancellationToken.None).ConfigureAwait(false);
            ...
            await DbConnection.ExecuteAsync("comment on function v10_comment_test_routine_2 (integer) is 'This is a test function.'", CancellationToken.None).ConfigureAwait(false);
        }

        [OneTimeTearDown]
        public async Task CleanUp()
        {
            await DbConnection.ExecuteAsync("drop function v10_comment_test_routine_1(integer)", CancellationToken.None).ConfigureAwait(false);
            await DbConnection.ExecuteAsync("drop function v10_comment_test_routine_2(integer)", CancellationToken.None).ConfigureAwait(false);
        }
        ...
        {
            var routineIsSome = await RoutineCommentProvider.GetRoutineComments("v10_comment_test_routine_1").IsSome.ConfigureAwait(false);
        ...
        {
            const string routineName = "v10_comment_test_routine_1";
        ...
        {
            var routineName = new Identifier("v10_comment_test_routine_1");
        ...
        {
            var routineName = new Identifier(IdentifierDefaults.Schema, "v10_comment_test_routine_1");
            var expectedRoutineName = new Identifier(IdentifierDefaults.Server, IdentifierDefaults.Database, IdentifierDefaults.Schema, "v10_comment_test_routine_1");
        ...
        {
            var routineName = new Identifier(IdentifierDefaults.Database, IdentifierDefaults.Schema, "v10_comment_test_routine_1");
            var expectedRoutineName = new Identifier(IdentifierDefaults.Server, IdentifierDefaults.Database, IdentifierDefaults.Schema, "v10_comment_test_routine_1");
        ...
        {
            var routineName = new Identifier("A", IdentifierDefaults.Database, IdentifierDefaults.Schema, "v10_comment_test_routine_1");
            var expectedRoutineName = new Identifier(IdentifierDefaults.Server, IdentifierDefaults.Database, IdentifierDefaults.Schema, "v10_comment_test_routine_1");
        ...
        public async Task GetAllRoutineComments_WhenEnumerated_ContainsTestRoutineComment()
        {
            var containsTestRoutine = await RoutineCommentProvider.GetAllRoutineComments()
                .AnyAsync(t => string.Equals(t.RoutineName.LocalName, "v10_comment_test_routine_1", StringComparison.Ordinal))
                .ConfigureAwait(false);
        ...
        {
            var comments = await GetRoutineCommentsAsync("v10_comment_test_routine_1").ConfigureAwait(false);
        ...
        public async Task GetRoutineComments_WhenFunctionContainsComment_ReturnsExpectedValue()
        {
            ...
            var comments = await GetRoutineCommentsAsync("v10_comment_test_routine_2").ConfigureAwait(false);
