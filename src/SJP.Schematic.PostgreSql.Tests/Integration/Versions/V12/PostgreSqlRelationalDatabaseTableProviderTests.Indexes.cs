...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V12
{
    internal sealed partial class PostgreSqlRelationalDatabaseTableProviderTests : PostgreSql12Test
    {
        ...
        {
            var table = await GetTableAsync("v12_table_test_table_1").ConfigureAwait(false);
        ...
        {
            var table = await GetTableAsync("v12_table_test_table_8").ConfigureAwait(false);
        ...
        public async Task Indexes_WhenGivenTableWithMultiColumnIndex_ReturnsIndexWithColumnsInCorrectOrder()
        {
            ...
            var table = await GetTableAsync("v12_table_test_table_9").ConfigureAwait(false);
            var index = table.Indexes.Single();
            var indexColumns = index.Columns
                .Select(c => c.DependentColumns.Single())
        ...
        {
            var table = await GetTableAsync("v12_table_test_table_11").ConfigureAwait(false);
            var index = table.Indexes.Single();
        ...
        {
            var table = await GetTableAsync("v12_table_test_table_13").ConfigureAwait(false);
            var index = table.Indexes.Single();
