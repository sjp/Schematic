...
LANGUAGE PLPGSQL").ConfigureAwait(false);
            ...
            await Connection.ExecuteAsync("comment on function v96_comment_test_routine_2 (integer) is 'This is a test function.'").ConfigureAwait(false);
        }

        [OneTimeTearDown]
        public async Task CleanUp()
        {
            await Connection.ExecuteAsync("drop function v96_comment_test_routine_1(integer)").ConfigureAwait(false);
            await Connection.ExecuteAsync("drop function v96_comment_test_routine_2(integer)").ConfigureAwait(false);
        }
        ...
        {
            var routineComments = await RoutineCommentProvider.GetAllRoutineComments().ConfigureAwait(false);
            var containsTestRoutine = routineComments.Any(t => t.RoutineName.LocalName == "v96_comment_test_routine_1");
