...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V9_6
{
    internal sealed class PostgreSqlDatabaseSequenceProviderTests : PostgreSql96Test
    {
        ...
        {
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_1", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_2 start with 20", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_3 start with 100 increment by 100", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_4 start with 1000 minvalue -99", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_5 start with 1000 no minvalue", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_6 start with 1 maxvalue 333", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_7 start with 1 no maxvalue", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_8 cycle", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_9 no cycle", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_10 cache 10", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create sequence v96_db_test_sequence_11 cache 1", CancellationToken.None).ConfigureAwait(false);
        }

        [OneTimeTearDown]
        public async Task CleanUp()
        {
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_1", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_2", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_3", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_4", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_5", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_6", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_7", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_8", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_9", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_10", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("drop sequence v96_db_test_sequence_11", CancellationToken.None).ConfigureAwait(false);
        }
