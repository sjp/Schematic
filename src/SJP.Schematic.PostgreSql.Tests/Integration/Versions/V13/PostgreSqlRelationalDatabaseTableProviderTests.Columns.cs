...
namespace SJP.Schematic.PostgreSql.Tests.Integration.Versions.V13
{
    internal partial class PostgreSqlRelationalDatabaseTableProviderTests : PostgreSql13Test
    {
        ...
        {
            const string tableName = "v13_table_test_table_1";
        ...
        {
            const string tableName = "v13_table_test_table_2";
            var table = await GetTableAsync(tableName).ConfigureAwait(false);
            var column = table.Columns.Single();
        ...
        {
            const string tableName = "v13_table_test_table_35";
            var table = await GetTableAsync(tableName).ConfigureAwait(false);
        ...
        {
            const string tableName = "v13_table_test_table_36";
            var table = await GetTableAsync(tableName).ConfigureAwait(false);
            var column = table.Columns.Last();
        ...
        {
            const string tableName = "v13_table_test_table_37";
            var table = await GetTableAsync(tableName).ConfigureAwait(false);
            var computedColumns = table.Columns.Where(c => c.IsComputed).ToList();
