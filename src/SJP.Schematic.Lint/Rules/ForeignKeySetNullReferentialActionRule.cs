...
/// <summary>
/// A linting rule which reports when a foreign key uses a <c>SET NULL</c> referential action but one or more of its columns are not nullable.
/// </summary>
/// <seealso cref="Rule"/>
/// <seealso cref="ITableRule"/>
public class ForeignKeySetNullReferentialActionRule : Rule, ITableRule
{
    /// <summary>
    /// The reporting level this rule uses unless a caller overrides it: warning, because
    /// SET NULL silently rewrites data on parent changes.
    /// </summary>
    public const RuleLevel DefaultLevel = RuleLevel.Warning;

    /// <summary>
    /// Initializes a new instance of the <see cref="ForeignKeySetNullReferentialActionRule"/> class.
    /// </summary>
    ...
    /// <summary>
    /// Analyses database tables. Reports messages when a foreign key has a <c>SET NULL</c> referential action applied to non-nullable columns.
    /// </summary>
    ...
    /// <summary>
    /// Analyses a database table. Reports messages when a foreign key has a <c>SET NULL</c> referential action applied to non-nullable columns.
    /// </summary>
    ...
    protected IReadOnlyCollection<IRuleMessage> AnalyseTable(IRelationalDatabaseTable table)
    {
        ...
        {
            var setsNull = foreignKey.DeleteAction == ReferentialAction.SetNull
                || foreignKey.UpdateAction == ReferentialAction.SetNull;
            if (!setsNull)
                continue;

            var hasNonNullableColumn = foreignKey.ChildKey.Columns.Any(static c => !c.IsNullable);
            if (!hasNonNullableColumn)
                continue;
    ...
    protected static string RuleId => "SCHEMATIC0025";

    /// <summary>
    /// Gets the rule title.
    /// </summary>
    /// <value>The rule title.</value>
    protected static string RuleTitle => "Foreign key with a SET NULL action on non-nullable columns.";
}
