...
namespace SJP.Schematic.Oracle
{
    ...
    {
        public OracleRelationalDatabaseTableProvider(IDbConnection connection, IDatabaseIdentifierDefaults identifierDefaults, IIdentifierResolutionStrategy identifierResolver, IDbTypeProvider typeProvider)
        {
...
order by t.OWNER, t.TABLE_NAME";
        ...
        public Option<Identifier> GetResolvedTableName(Identifier tableName)
        {
            if (tableName == null)
                throw new ArgumentNullException(nameof(tableName));
            ...
            return resolvedNames
                .Select(GetResolvedTableNameStrict)
                .FirstSome();
        }
...
where
    ...
    and o.SECONDARY <> 'Y'";
        ...
        protected virtual Option<IRelationalDatabaseTable> LoadTableSync(Identifier tableName)
        {
            ...
            var checks = LoadChecksSync(resolvedTableName, columnLookup);
        ...
        protected virtual IDatabaseKey LoadPrimaryKeySync(Identifier tableName, IReadOnlyDictionary<Identifier, IDatabaseColumn> columns)
        {
            ...
            return new OracleDatabaseKey(constraintName, DatabaseKeyType.Primary, keyColumns, isEnabled);
        }
...
where pac.OWNER = :SchemaName and pac.TABLE_NAME = :TableName and ac.CONSTRAINT_TYPE = 'R' and pac.CONSTRAINT_TYPE in ('P', 'U')";
        ...
        protected virtual IReadOnlyCollection<IDatabaseCheckConstraint> LoadChecksSync(Identifier tableName, IReadOnlyDictionary<Identifier, IDatabaseColumn> columns)
        {
            if (tableName == null)
                throw new ArgumentNullException(nameof(tableName));
            if (columns == null)
...
where ac.OWNER = :SchemaName and ac.TABLE_NAME = :TableName and ac.CONSTRAINT_TYPE = 'R' and pac.CONSTRAINT_TYPE in ('P', 'U')";
        ...
        protected virtual IReadOnlyList<IDatabaseColumn> LoadColumnsSync(Identifier tableName)
        {
            ...
            var notNullableColumnNames = GetNotNullConstrainedColumns(tableName, columnNames);
        ...
        private async Task<IReadOnlyList<IDatabaseColumn>> LoadColumnsAsyncCore(Identifier tableName, CancellationToken cancellationToken)
        {
            ...
            var columnNames = query.Select(row => row.ColumnName).ToList();
            var notNullableColumnNames = await GetNotNullConstrainedColumnsAsync(tableName, columnNames).ConfigureAwait(false);
            var result = new List<IDatabaseColumn>();
...
where TABLE_OWNER = :SchemaName and TABLE_NAME = :TableName and BASE_OBJECT_TYPE = 'TABLE'";
        ...
        protected Task<IEnumerable<string>> GetNotNullConstrainedColumnsAsync(Identifier tableName, IEnumerable<string> columnNames)
        {
            if (tableName == null)
                throw new ArgumentNullException(nameof(tableName));
            if (columnNames == null)
            ...
            return GetNotNullConstrainedColumnsAsyncCore(tableName, columnNames);
        }

        private async Task<IEnumerable<string>> GetNotNullConstrainedColumnsAsyncCore(Identifier tableName, IEnumerable<string> columnNames)
        {
            var checks = await Connection.QueryAsync<CheckConstraintData>(ChecksQuery, new { SchemaName = tableName.Schema, TableName = tableName.LocalName }).ConfigureAwait(false);
            if (checks.Empty())
                return Array.Empty<string>();
