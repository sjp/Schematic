...
public class OracleRelationalDatabaseTableProvider : IRelationalDatabaseTableProvider
{
    ...
    protected OracleTableQueryCache CreateQueryCache(CancellationToken cancellationToken) => new(
        new AsyncCache<Identifier, Option<Identifier>, OracleTableQueryCache>((tableName, _, token) => GetResolvedTableNameStrict(tableName, token).ToOption(), cancellationToken),
    ...
    private async Task<IReadOnlyList<Identifier>> LoadTableNamesAsync(OracleTableQueryCache queryCache, CancellationToken cancellationToken)
    {
    ...
    private async Task<Option<Identifier>> ResolveTableNameAsync(Identifier tableName, OracleTableQueryCache queryCache, CancellationToken cancellationToken)
    {
        // Each candidate is looked up exactly, through the query cache and under the same key that foreign-key
        // loading uses. When the table references itself or has child tables, their lookups of this table's
        // name then find the result instead of querying for it again.
