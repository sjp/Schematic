...
namespace SJP.Schematic.Sqlite.Tests.Integration
{
    internal partial class SqliteRelationalDatabaseTableProviderTests : SqliteTest
    {
        ...
        {
            await Connection.ExecuteAsync("create table db_test_table_1 (id integer)", CancellationToken.None).ConfigureAwait(false);
...
create table table_test_table_7 (
    ...
    constraint uk_test_table_7 unique (first_name, last_name, middle_name)
            ...
            await Connection.ExecuteAsync("create table table_test_table_8 ( test_column int )", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create index ix_test_table_8 on table_test_table_8 (test_column)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
create table table_test_table_9 (
    ...
    last_name nvarchar(50)
            ...
            await Connection.ExecuteAsync("create index ix_test_table_9 on table_test_table_9 (first_name, last_name, middle_name)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync(@"
...
create table table_test_table_32 (
    ...
    constraint ck_test_table_32 check ([test_column]>(1))
            ...
            await Connection.ExecuteAsync("create table trigger_test_table_1 (table_id integer primary key not null)", CancellationToken.None).ConfigureAwait(false);
            await Connection.ExecuteAsync("create table trigger_test_table_2 (table_id integer primary key not null)", CancellationToken.None).ConfigureAwait(false);
