...
[TestFixture]
internal static class SqliteDependencyProviderTests
{
    [Test]
    public static void Ctor_GivenNullComparer_CreatesWithoutError()
    {
        Assert.That(() => new SqliteDependencyProvider(null), Throws.Nothing);
    }

    [Test]
    public static void GetDependencies_GivenNullObjectName_ThrowsArgumentNullException()
    {
    ...
    [TestCase("")]
    [TestCase("    ")]
    public static void GetDependencies_GivenNullOrWhiteSpaceExpression_ThrowsArgumentException(string expression)
    {
        var provider = new SqliteDependencyProvider();
    ...
    [Test]
    public static void GetDependencies_GivenExpressionPointingToQualifiedName_ReturnsQualifiedIdentifier()
    {
        var provider = new SqliteDependencyProvider();
        Identifier objectName = "test";
        const string expression = "select * from main.other_table";

        var dependencies = provider.GetDependencies(objectName, expression);
        var dependency = dependencies.Single();

        Assert.That(dependency, Is.EqualTo(new Identifier("main", "other_table")));
    }
